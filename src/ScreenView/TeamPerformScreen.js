...
import {setTeamMembersData} from '../store/projectSlice';
import LoadingModal from '../Components/LoadingModal';

const TeamPerformScreen = ({navigation, route}) => {
  ...
  const {myOrdersData, allSupervisorsData, allTeamsData, employsData} =
  ...
  useEffect(() => {
    ...
    getTotalRevOrder();
  }, [fromDateValue, toDateValue, teamId, myOrdersData]);
  ...
  useEffect(() => {
    const getTeamData = async () => {
      setisLoading(true);
      const filteredMembers = employsData?.filter(dat => dat?.id === teamId);
      setmemebersArr(filteredMembers[0]?.teamInfo?.members);
      setisLoading(false);
    };
    getTeamData();
  }, [employsData, teamId]);
  return (
    <>
      <View style={styles.fillscreenbg}>
        <CurvedHeaderComp
          name={textStrings.teamPerformaanceTxt}
          iconName1={'left'}
                ...
                <View
                  ...
                  <View
                    ...
                    <MainBtnComp
                      ...
                      subtitle={textStrings.teamEvolutionTxt}
                      title={`${allTeamsData[0]?.value}`}
                    />
                  </View>
...
export default TeamPerformScreen;
